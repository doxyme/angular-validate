        ...
        .provider('$validation', function() {
            ...
            this.validate = function(form) {
                ...
                Object.keys(form).forEach(function(key) {
                  if (key[0] === '$' || form[key].$dirty) { return; }
                  form[key].$setTouched();
                  if (form[key].$setViewValue) {
                    return form[key].$setViewValue(form[key].$viewValue);
                  }
                });
                ...
                var isValid = _this.checkValid(form);
                $timeout(function() {
                    if (isValid) {
...
(function() {
    'use strict';
    angular.module('validation.directive', ['validation.provider', 'ngAnimate'])
        .directive('validator', ['$injector', '$animate',
            function($injector, $animate) {
                ...
                function debounce(func, threshold) {
                  ...
                  return function debounced() {
                    ...
                    promise = $timeout(delayed, threshold || 100);
                  };
                }
                var toggleEnter = debounce(function(container, html) {
                  var currentContent = container.children();
                  if (html && currentContent.length) {
                    container.html(html);
                  } else if (html) {
                    $animate.enter(html, container);
                  } else if (currentContent.length) {
                    $animate.leave(currentContent);
                  }
                });
                ...
                var validFunc = function(element, validMessage, validation, scope, ctrl) {
                    ...
                    if ($validationProvider.showSuccessMessage && messageToShow) {
                        html = $validationProvider.getSuccessHTML(messageToShow);
                        // messageElem.html($validationProvider.getSuccessHTML(messageToShow));
                        // messageElem.css('display', '');
                    } else {
                        // messageElem.css('display', 'none');
                    }
                    toggleEnter(messageElem, html);
                ...
                var invalidFunc = function(element, validMessage, validation, scope, ctrl) {
                    ...
                    if ($validationProvider.showErrorMessage && messageToShow) {
                        html = $validationProvider.getErrorHTML(messageToShow);
